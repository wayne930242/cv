...
1. 響應式編程（Reactive Programming）：響應式編程透過定義數據流和自動處理的依賴關係，讓系統能夠即時回應數據或狀態的變化。
2. 函數式編程（Functional Programming）：函數式編程以不可變數據和純函數為核心，強調函數作為一等公民並避免副作用。
...
如果只想處理回傳值，Observable 可以透過一個回呼函數訂閱。此時，訂閱者所散播的錯誤和完成的訊息會直接被忽略：
...
2. 當在 Subject 上註冊了一個清單的 Observer，Observable 能夠訂閱它。
...
2. `bindCallback`: 將回調 API 轉換為返回 Observable 的函數。參數：要轉換的回調函數。
3. `bindNodeCallback`: 將 Node.js 風格的回調 API 轉換為返回 Observable 的函數。參數：要轉換的 Node.js 風格的回調函數。
...
6. `observeOn`: 指定 Observable 在哪個調度器上發出通知。
7. `subscribeOn`: 指定 Observable 在哪個調度器上進行訂閱。
