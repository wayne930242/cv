...
除了直接在函式宣告時定義參數和返回值的型別，我們還可以把函式的型別獨立定義，只是這時候始能使用函式表達式或是箭頭函式：
...
`TypeScript` 會自動判斷出 `return` 的型別，因此我們可以省略 `return` 型別的宣告：
  ...
  if (typeof data === 'object' && data !== null) {
    ...
    if ('name' in data && 'age' in data) {
      ...
      if (typeof name === 'string' && typeof age === 'number') {
        // 到了這一行，TypeScript 知道 name 是字串，並且 age 是數字。
        return { name, age }
      }
    }
  }
...
這樣我們該怎麼寫呢？這時候我們就該使用泛型，語法如下。在這樣的情況下，`TypeScript` 會根據呼叫的時候傳入的參數的型別來判斷 `T` 是什麼：
...
這個 `T` 的命名是常見慣例，但並不是強硬的，你可以使用任何的非保留字作為泛型的字元。
...
// 在這個例子中，我刻意省略了 return 的型別宣告，但你也可以明示它：T & U。
```
