...
`TypeScript` 是 `JavaScript` 的超集：它是 `JavaScript` 的語法擴充。

原則上，所有 `JavaScript` 語法都能在 `TypeScript` 中沿用。但實際上，並非所有的 `JavaScript` 語法都是合法的。它會給予語法額外的嚴格檢查。嚴格性限制了 `JavaScript` 的自由，並為程式碼提供更高的安全性。
...
這裡有個重要的觀念：對於 `TypeScript` 來說，型別的意義，在於這個對象應該擁有哪些屬性和方法，而不在於它叫什麼名字。我們可以將屬性和方法桶稱為「結構」，因此這樣的型別又叫「結構性型別」（或有人會叫它「鴨子型別」）。

如果你想寫 `TypeScript`，你需要調整 `JavaScript` 專案的語言配置。但你可以在 [TypeScript Playground](https://www.typescriptlang.org/play/) 上進行實驗。
...
如果我們有兩個同樣的型別，一個是 `state1`，另一個是 `state2`，我們可以這樣寫：
...
(2) 此外，它能夠使用 `extends` 來進行擴充（但它不能使用二元運算子）：
...
如果使用 `type`，可以改用下面這段語法等價語法（`&` 表示將兩個型別的屬性和方法加在一起，這裡可以多想一下把心智模式建立起來）：
...
宣告合併這個特性是 `type` 沒有的，而這個特性廣泛用於第三方模組或是全局類型的擴充上。在這裡可以先記住這個特性，你需要在實作中去看到這特性的好處。
